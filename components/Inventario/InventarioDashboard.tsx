...
import {
  Container, Typography, Box, Tabs, Tab, Paper, Button, Alert, CircularProgress,
  Dialog, DialogTitle, DialogContent, DialogActions, TextField, MenuItem,
  FormControlLabel, Switch, IconButton, Tooltip, Snackbar, Stack,
} from '@mui/material';
...
const InventarioDashboard: React.FC = () => {
  ...
  const [ubicacionForm, setUbicacionForm] = useState({
    codigo: '', nombre: '', tipo: '', ubicacionPadreId: undefined as number | undefined,
    capacidadMaxima: undefined as number | undefined, activo: true,
  });
  ...
  const [proveedorForm, setProveedorForm] = useState({
    nombre: '', cedula: '', ruc: '', direccion: '', telefono: '', email: '',
    contactoPrincipal: '', plazoEntregaDias: 7, activo: true,
  });
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      setUbicaciones(ubis); setProveedores(provs); setProductos(prods);
    } catch {
  ...
  const handleOpenUbicacionDialog = (ubicacion?: Ubicacion) => {
    if (ubicacion) {
      ...
      setUbicacionForm({
        codigo: ubicacion.codigo, nombre: ubicacion.nombre, tipo: ubicacion.tipo || '',
        ubicacionPadreId: ubicacion.ubicacionPadreId || undefined,
        capacidadMaxima: ubicacion.capacidadMaxima || undefined, activo: ubicacion.activo,
      });
    } else {
      setEditUbicacion(null);
      setUbicacionForm({ codigo: '', nombre: '', tipo: '', ubicacionPadreId: undefined, capacidadMaxima: undefined, activo: true });
    }
  ...
  const handleSaveUbicacion = async () => {
    setLoading(true);
    try {
      if (editUbicacion) await inventarioService.updateUbicacion({ id: editUbicacion.id, ...ubicacionForm });
      else await inventarioService.createUbicacion(ubicacionForm);
      setSnackbarMessage('Ubicación guardada'); setSnackbarSeverity('success'); setSnackbarOpen(true);
      setOpenUbicacionDialog(false); cargarDatos();
    } catch (err: any) {
  ...
  const handleDeleteUbicacion = async (id: number) => {
    ...
    try {
      ...
      setSnackbarMessage('Ubicación eliminada'); setSnackbarSeverity('success'); setSnackbarOpen(true);
      cargarDatos();
    } catch (err: any) {
      setSnackbarMessage(err.response?.data?.message || 'Error'); setSnackbarSeverity('error'); setSnackbarOpen(true);
    } finally { setLoading(false); }
  ...
  const handleOpenProveedorDialog = (proveedor?: Proveedor) => {
    if (proveedor) {
      ...
      setProveedorForm({
        nombre: proveedor.nombre, cedula: proveedor.cedula, ruc: proveedor.ruc,
        direccion: proveedor.direccion || '', telefono: proveedor.telefono || '',
        email: proveedor.email || '', contactoPrincipal: proveedor.contactoPrincipal || '',
        plazoEntregaDias: proveedor.plazoEntregaDias || 7, activo: proveedor.activo,
      });
    } else {
      setEditProveedor(null);
      setProveedorForm({ nombre: '', cedula: '', ruc: '', direccion: '', telefono: '', email: '', contactoPrincipal: '', plazoEntregaDias: 7, activo: true });
    }
  ...
  const handleSaveProveedor = async () => {
    setLoading(true);
    try {
      if (editProveedor) await inventarioService.updateProveedor({ id: editProveedor.id, ...proveedorForm });
      else await inventarioService.createProveedor(proveedorForm);
      setSnackbarMessage('Proveedor guardado'); setSnackbarSeverity('success'); setSnackbarOpen(true);
      setOpenProveedorDialog(false); cargarDatos();
    } catch (err: any) {
      setSnackbarMessage(err.response?.data?.message || 'Error'); setSnackbarSeverity('error'); setSnackbarOpen(true);
    } finally { setLoading(false); }
  };
  ...
  const handleToggleProveedor = async (id: number, activo: boolean) => {
    try {
      ...
      setSnackbarMessage('Error al cambiar estado'); setSnackbarSeverity('error'); setSnackbarOpen(true);
    }
  };
  ...
  const handleDeleteProveedor = async (id: number, nombre: string) => {
    ...
    try {
      ...
      setSnackbarMessage(`Proveedor "${nombre}" eliminado`); setSnackbarSeverity('success'); setSnackbarOpen(true);
      cargarDatos();
    } catch (err: any) {
      setSnackbarMessage(err.response?.data?.message || 'Error'); setSnackbarSeverity('error'); setSnackbarOpen(true);
    } finally { setLoading(false); }
    ...
    <Container maxWidth="xl" sx={{ px: { xs: 1, sm: 2, md: 3 } }}>
      <Box sx={{
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
          <Button startIcon={<Refresh />} onClick={cargarDatos} variant="outlined" fullWidth={false}>Refrescar</Button>
          <Button variant="contained" startIcon={<Add />} onClick={() => handleOpenUbicacionDialog()} fullWidth={false}>
            Nueva Ubicación
          </Button>
        </Stack>
      </Box>
      <Paper sx={{ width: '100%' }}>
        ...
        <TabPanel value={tabValue} index={0}>
          <UbicacionesList ubicaciones={ubicaciones} onEdit={handleOpenUbicacionDialog} onDelete={handleDeleteUbicacion} />
        </TabPanel>
        <TabPanel value={tabValue} index={1}><MovimientosList productos={productos} /></TabPanel>
        <TabPanel value={tabValue} index={2}>
          <EntradaProductoForm productos={productos} proveedores={proveedores} ubicaciones={ubicaciones}
            onSuccess={() => {
              setSnackbarMessage('Entrada registrada'); setSnackbarSeverity('success'); setSnackbarOpen(true);
              cargarDatos();
            }} />
        </TabPanel>
        <TabPanel value={tabValue} index={3}>
          <NumerosSerieList productos={productos} ubicaciones={ubicaciones} onRefresh={cargarDatos} />
        </TabPanel>
        <TabPanel value={tabValue} index={4}>
          <Box sx={{
            mb: 2, display: 'flex', flexDirection: { xs: 'column', sm: 'row' },
            justifyContent: 'space-between', alignItems: { xs: 'stretch', sm: 'center' }, gap: 2,
          }}>
            <FormControlLabel
              control={<Switch checked={mostrarInactivos} onChange={(e) => setMostrarInactivos(e.target.checked)} />}
              label="Mostrar inactivos" />
            <Button variant="contained" startIcon={<Add />} onClick={() => handleOpenProveedorDialog()}
              sx={{ width: { xs: '100%', sm: 'auto' } }}>Nuevo Proveedor</Button>
          </Box>
          {proveedores.map(p => (
            <Paper key={p.id} sx={{
              ...
              <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <IconButton size="small" color="error" onClick={() => handleDeleteProveedor(p.id, p.nombre)}>
                  <Delete />
                </IconButton>
                <FormControlLabel
      ...
      <Dialog open={openUbicacionDialog} onClose={() => setOpenUbicacionDialog(false)}
        maxWidth="sm" fullWidth fullScreen={esMovil}>
        <DialogTitle>{editUbicacion ? 'Editar Ubicación' : 'Nueva Ubicación'}</DialogTitle>
        <DialogContent>
          <TextField label="Código" fullWidth margin="normal" value={ubicacionForm.codigo}
            onChange={(e) => setUbicacionForm({ ...ubicacionForm, codigo: e.target.value })} required />
          <TextField label="Nombre" fullWidth margin="normal" value={ubicacionForm.nombre}
            onChange={(e) => setUbicacionForm({ ...ubicacionForm, nombre: e.target.value })} required />
          <TextField label="Tipo" select fullWidth margin="normal" value={ubicacionForm.tipo}
          ...
          <TextField label="Ubicación Padre" select fullWidth margin="normal" value={ubicacionForm.ubicacionPadreId || ''}
          ...
          <TextField label="Capacidad Máxima" type="number" fullWidth margin="normal"
      ...
      <Dialog open={openProveedorDialog} onClose={() => setOpenProveedorDialog(false)}
        maxWidth="sm" fullWidth fullScreen={esMovil}>
        ...
        <DialogContent>
          ...
          <TextField label="Plazo Entrega (días)" type="number" fullWidth margin="normal" value={proveedorForm.plazoEntregaDias}
            onChange={(e) => setProveedorForm({ ...proveedorForm, plazoEntregaDias: Number(e.target.value) })} />
